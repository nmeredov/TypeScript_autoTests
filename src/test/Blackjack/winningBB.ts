...
import * as body from '../../RBOW_UAT_API3_Games/BlackJack/winningBBBody'; 
...
describe("Get balance before setllement", () => {
  ...
  beforeAll(async () => {
    ...
    currentBalance = data.balances[0].amount;
    initialBalance = currentBalance + (variable.bet1 *16) + (variable.bet2 * 7);
    expectedBalance = initialBalance - ((variable.bet1 * 16) + (variable.bet2 * 7));
...
describe("Final Settlement - Checks if response status is successful", () => {
  ...
  beforeAll(async () => {
    ...
    console.log('Payout is:', (variable.payoff * 14) + variable.payoff2 + variable.payoff3 + (variable.payoff4 * 7));
...
describe("Get balance", () => {
  ...
  beforeAll(async () => {
    ...
    data = await testResponse.data;
    currentBalance = data.balances[0].amount;
    initialBalance = currentBalance + ((variable.bet1 * 16) + (variable.bet2 * 7)) - (variable.payoff * 14) - variable.payoff2 - variable.payoff3 - (variable.payoff4 * 7);
    expectedBalance = initialBalance +(variable.payoff * 14) + variable.payoff2 + variable.payoff3 + (variable.payoff4 * 7) - ((variable.bet1 * 16) + (variable.bet2 * 7));
    console.log('Initial balance:', initialBalance);
    console.log('Current balance:', currentBalance);
    console.log('Expected balance:', expectedBalance);
    console.log('get_balance_request', body.getBalanceBody);
